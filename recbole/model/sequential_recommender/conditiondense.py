...
class ConditionDense(SequentialRecommender):

    def __init__(self, config, dataset):
        super(ConditionDense, self).__init__(config, dataset)
        ...
        if config['item_transformer_layer'] == 1:
            self.item_feature_transform=torch.nn.Sequential(nn.LayerNorm(self.hidden_size + self.side_feature_size),
                                    nn.Linear(self.hidden_size + self.side_feature_size, self.hidden_size),
                                    nn.ReLU(),
                    )
        ...
        # if self.side_feature_size == 1:
        #     self.item_interaction_feature = self.item_interaction_feature.unsqueeze(1)
        self.side_dense_layer = nn.Linear(self.hidden_size, self.hidden_size)
        ...
        if config["freeze_Rec_Params"]:
            for p_name, param in self.named_parameters():
                ...
                if 'adapter' in p_name or 'prefix' in p_name or 'item_feature_transform' in p_name or "side_dense_layer" in p_name:
    ...
    def forward(self, item_seq, item_seq_len, item_additional_embedding=None):
        ...
        # SASRec Encoder
        output = self.SASRecEncoder(item_seq, item_seq_len, item_additional_embedding, adapter, prefix, item_feature_transform, self.side_dense_layer)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.item_transformer_mode == 1:
            ...
            item_additional_embedding = self.side_embedding
        ...
        if self.loss_type == "BPR":
            ...
            if self.item_feature_transform and self.item_transformer_mode == 2:
                pos_items_additional_feature = self.side_embedding[pos_items]
                neg_items_additional_feature = self.side_embedding[neg_items]
            ...
            if self.item_feature_transform and self.item_transformer_mode == 2:
                test_item_emb = self.item_feature_transform(torch.cat((test_item_emb, self.side_embedding), dim=1))
    ...
    def predict(self, interaction):
        ...
        if self.item_feature_transform and self.item_transformer_mode == 2:
            test_items_additional_feature = self.side_embedding[test_item]
...
class SASRecEncoder(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len, item_additional_embedding=None, adapter=None, prefix=None, item_feature_transform=None, side_dense_layer=None):
        ...
        output = side_dense_layer(output)
