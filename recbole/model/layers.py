...
class MultiHeadAttention_Prefix(nn.Module):
    """
    Multi-head Self-attention layers, a attention score dropout layer is introduced.
    ...
    def __init__(
        ...
        super(MultiHeadAttention_Prefix, self).__init__()
        if hidden_size % n_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (hidden_size, n_heads)
    ...
    def forward_hypter_prefix(self, input_tensor, attention_mask, prefix_q, prefix_k):
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)

        # Add prefix into Attention Layer
        mixed_query_layer = torch.cat((prefix_q[:,:,0,:], mixed_query_layer), dim=1)
        mixed_key_layer = torch.cat((prefix_k[:,:,0,:], mixed_key_layer), dim=1)
        ...
        attention_probs = attention_probs[:, :, prefix_len:, prefix_len:]
...
class TransformerLayer_Prefix_Adapter(nn.Module):
    """
    One transformer layer consists of a multi-head self-attention layer and a point-wise feed-forward layer.
    ...
    def __init__(
        ...
        super(TransformerLayer_Prefix_Adapter, self).__init__()
        self.multi_head_attention = MultiHeadAttention_Prefix(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps
        )
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        attention_output = self.layer_norm1(self.dropout1(attention_output) + hidden_states)
        output = self.linear1(attention_output).relu()
        output = self.linear2(output)
        output = self.layer_norm2(self.dropout2(output) + attention_output)
        return output
    
    def forward_adapter(self, hidden_states, attention_mask, adapter1=None,adapter2=None):
        attention_output = self.multi_head_attention(hidden_states, attention_mask)
        
        attention_output = self.dropout1(attention_output)
        attention_output = adapter1(attention_output) + attention_output
    ...
    def forward_adapter_prefix(self, hidden_states, attention_mask, adapter1=None,adapter2=None, prefix=None):
        prefix_len = prefix[0].size(1)
        attention_mask = torch.nn.functional.pad(attention_mask,(prefix_len,0,prefix_len,0),value=0.)
        attention_output = self.multi_head_attention.forward_hypter_prefix(hidden_states, attention_mask, prefix_q=prefix[0], prefix_k=prefix[1])
    ...
    def forward_prefix(self, hidden_states, attention_mask, prefix=None):
        attention_output = self.multi_head_attention.forward_hypter_prefix(hidden_states, attention_mask)
